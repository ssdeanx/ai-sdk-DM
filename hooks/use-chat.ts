...
export function useChat({
  ...
  const [streamableValues, setStreamableValues] = useState<
          ...
          try {
            ...
            while (true) {
              ...
              if (chunk.includes('__STREAMABLE_UPDATE__')) {
                try {
                  const streamableUpdate = JSON.parse(
                    chunk.split('__STREAMABLE_UPDATE__')[1]
                  );
                  if (
                    streamableUpdate &&
                    streamableUpdate.key &&
                    streamables?.[streamableUpdate.key]
                  ) {
                    ...
                    streamables[streamableUpdate.key].onUpdate?.(
                      streamableUpdate.value
                    );
                  }
                } catch (e) {
  ...
  const runSequentialGenerations = useCallback(
    async (
      prompts: string[],
      options?: {
        modelId?: string;
