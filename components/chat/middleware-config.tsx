  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
          ...
          {caching.enabled && (
            <div className="space-y-2 pl-4 mt-2">
              <div className="space-y-1">
                {' '}
                <Label htmlFor="caching-ttl">
                  Cache TTL (ms): {caching.ttl}
                </Label>
                <Slider
                  id="caching-ttl"
                  min={1000}
                  ...
                  value={[caching.ttl]}
                  onValueChange={(value) =>
                    onChange({ caching: { ...caching, ttl: value[0] } })
                  }
                />
              </div>{' '}
              <div className="space-y-1">
                <Label htmlFor="caching-max-size">
                  Max Cache Size: {caching.maxSize}
                </Label>{' '}
                <Slider
                  id="caching-max-size"
                  min={10}
        ...
        {/* Similar sections for reasoning, simulation, and logging */}{' '}
      </CardContent>
    </Card>
  );
}
