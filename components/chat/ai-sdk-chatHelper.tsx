...
import {
  ...
  type VisualizationWithTracingProps,
} from './visualization-with-tracing';
import {
  DataTable,
...
import {
  InteractiveMap,
  type Location as InteractiveMapLocation,
  type InteractiveMapProps,
} from './interactive-map';
import {
  InteractiveForm,
  type FormField as InteractiveFormField,
...
const ComputerUsePropsSchema: z.ZodType<Omit<ComputerUseProps, 'onRun'>> =
  z.object({
...
const DataSeriesSchema: z.ZodType<DataSeries> = z.object({
  name: z.string(),
  data: z.union([z.array(DataPointSchema), z.array(z.number())]),
  color: z.string().optional(),
  type: z.string().optional(),
});

const DataVisualizationPropsSchema: z.ZodType<DataVisualizationProps> =
  z.object({
...
const VisualizationWithTracingPropsSchema: z.ZodType<VisualizationWithTracingProps> =
  z.object({
    title: z.string().optional(),
    data: z.union([
...
const DataTableColumnSchema: z.ZodType<
  Omit<DataTableColumn, 'render'> & { render?: undefined }
> = z.object({
  key: z.string(),
  title: z.string(),
...
const DataTablePropsSchema: z.ZodType<
  Omit<DataTableProps, 'columns'> & {
    columns: Array<Omit<DataTableColumn, 'render'>>;
  }
> = z.object({
  title: z.string().optional(),
...
const InteractiveMapLocationSchema: z.ZodType<InteractiveMapLocation> =
  z.object({
    lat: z.number(),
    lng: z.number(),
    title: z.string().optional(),
...
const InteractiveFormFieldSchema: z.ZodType<
  Omit<InteractiveFormField, 'validation'> & {
    validation?: {
      pattern?: string;
      min?: number;
      max?: number;
...
const InteractiveFormPropsSchema: z.ZodType<
  Omit<InteractiveFormProps, 'onSubmit' | 'onCancel'> & {
    onSubmit?: undefined;
    onCancel?: undefined;
  }
> = z.object({
...
const ParseErrorDisplay: React.FC<ParseErrorDisplayProps> = ({
  componentName,
  errorMessage,
  originalText,
  validationErrors,
...
export function renderContent(content: string): React.ReactNode {
  ...
  const customRegexes = [
    ...
    {
      regex:
        /<ComputerUse\s+task="([^"]+)"(?:\s+showSteps="(true|false)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        const props = {
          title: m[1],
          content: m[1],
    ...
    {
      regex:
        /<DataVisualization\s+data='([^']+)'(?:\s+type="([^"]+)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        try {
          ...
          return (
            <ParseErrorDisplay
              ...
              errorMessage="Invalid props after JSON parse."
              validationErrors={result.error.format()}
              originalText={m[0]}
            />
          );
          ...
          return (
            <ParseErrorDisplay
              key={key}
              componentName="DataVisualization"
              errorMessage={`JSON parsing error: ${message}`}
              originalText={m[0]}
            />
          );
        }
    ...
    {
      regex:
        /<VisualizationWithTracing\s+data='([^']+)'(?:\s+type="([^"]+)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        try {
          const jsonData = JSON.parse(m[1]);
          const props = { data: jsonData, type: m[2] };
          ...
          return (
            <ParseErrorDisplay
              key={key}
              componentName="VisualizationWithTracing"
              errorMessage="Invalid props after JSON parse."
              validationErrors={result.error.format()}
              originalText={m[0]}
            />
    ...
    {
      regex:
        /<ScreenShare\s+src="([^"]+)"(?:\s+title="([^"]*)")?(?:\s+isVideo="(true|false)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        const props = {
          src: m[1],
    ...
    {
      regex:
        /<InteractiveMap\s+center="\[\s*([0-9.-]+)\s*,\s*([0-9.-]+)\s*\]"(?:\s+zoom="(\d+)")?(?:\s+locations='([^']*)')?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        try {
          ...
          return (
            <ParseErrorDisplay
              ...
              errorMessage="Invalid props after parsing."
              validationErrors={result.error.format()}
              originalText={m[0]}
            />
          );
          ...
          return (
            <ParseErrorDisplay
              key={key}
              componentName="InteractiveMap"
              errorMessage={`Attribute parsing error: ${message}`}
              originalText={m[0]}
            />
          );
        }
    ...
    {
      regex:
        /<InteractiveForm\s+title="([^"]+)"(?:\s+fields='([^']*)')?(?:\s+submitLabel="([^"]*)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        try {
    ...
    {
      regex:
        /<AudioPlayer\s+src="([^"]+)"(?:\s+title="([^"]*)")?(?:\s+waveform="(true|false)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        const props = {
          src: m[1],
          title: m[2],
    ...
    {
      regex:
        /<ModelViewer\s+modelUrl="([^"]+)"(?:\s+title="([^"]*)")?(?:\s+format="(gltf|glb|obj|stl)")?(?:\s+autoRotate="(true|false)")?(?:\s+backgroundColor="([^"]*)")?(?:[^>]*)\/?>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        const props = {
          modelUrl: m[1],
          title: m[2],
    ...
    {
      regex:
        /<CanvasDisplay(?:\s+width="(\d+)")?(?:\s+height="(\d+)")?(?:\s+className="([^"]*)")?\s*>([\s\S]*?)<\/CanvasDisplay>/g,
      render: (m: RegExpExecArray, key: string | number) => {
        ...
        if (result.success) {
          return (
            <CanvasDisplay key={key} {...result.data}>
              {children}
            </CanvasDisplay>
          );
        }
  ...
  while (lastIndexProcessed < content.length) {
    let earliestMatch: {
      ...
      renderFunc: (m: RegExpExecArray, key: string | number) => React.ReactNode;
      regex: RegExp;
    } | null = null;
    ...
    if (earliestMatch) {
      ...
      parts.push(
        earliestMatch.renderFunc(
          earliestMatch.match,
          `component-${componentKeyIndex++}`
        )
      );
      lastIndexProcessed =
  ...
  if (remainingTextAfterCustom) {
    ...
    while (
      (codeMatch = codeBlockRegex.exec(remainingTextAfterCustom)) !== null
    ) {
      if (codeMatch.index > lastCodeBlockIndex) {
        parts.push(
          <p
            key={`text-code-${componentKeyIndex++}`}
            className="whitespace-pre-wrap"
          >
            {remainingTextAfterCustom.slice(
              lastCodeBlockIndex,
              codeMatch.index
            )}
          </p>
      ...
      parts.push(
        lang === 'mermaid' ? (
          <MermaidDiagram
            key={`mermaid-${componentKeyIndex++}`}
            code={codeContent}
          />
        ) : (
          <CodeBlock
            key={`codeblock-${componentKeyIndex++}`}
            language={lang}
            code={codeContent}
          />
        )
    ...
    if (lastCodeBlockIndex < remainingTextAfterCustom.length) {
      parts.push(
        <p
          key={`text-final-${componentKeyIndex++}`}
          className="whitespace-pre-wrap"
        >
    ...
    parts.push(
      <p
        key={`fulltext-${componentKeyIndex++}`}
        className="whitespace-pre-wrap"
      >
        {content}
      </p>
  ...
  return parts.length > 0 ? (
    ...
    <p key="empty" className="whitespace-pre-wrap"></p>
  );
}
