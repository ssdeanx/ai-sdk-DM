...
export function DatabaseStatus({
  ...
  >('loading');
  const [libsqlStatus, setLibsqlStatus] = useState<
    'loading' | 'connected' | 'error'
  >('loading');
  const [upstashStatus, setUpstashStatus] = useState<
    'loading' | 'connected' | 'error'
  >('loading');
  const [upstashStats, setUpstashStats] = useState<{
    info?: unknown;
    error?: string;
  } | null>(null);
        ...
        <TooltipContent>
          <div className="space-y-2 p-1">
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={
                  supabaseStatus === 'connected' ? 'default' : 'destructive'
                }
              >
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={
                  libsqlStatus === 'connected' ? 'default' : 'destructive'
                }
              >
                {libsqlStatus === 'loading'
                  ? 'Checking...'
                  : libsqlStatus === 'connected'
                    ? 'Connected'
                    : 'Error'}
              </Badge>
            </div>
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={
                  upstashStatus === 'connected' ? 'default' : 'destructive'
                }
              >
                {upstashStatus === 'loading'
                  ? 'Checking...'
            ...
            {upstashStats?.error && (
              <div className="text-xs text-destructive pt-1">
                Upstash error: {upstashStats.error}
              </div>
            )}
            {lastChecked && (
