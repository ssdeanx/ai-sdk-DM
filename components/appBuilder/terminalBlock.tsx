...
export const AppBuilderTerminalBlock: React.FC<AppBuilderTerminalBlockProps> = ({ content = '', className = '', onCommand, interactive = false }) => {
  ...
  useEffect(() => {
    if (containerRef.current && !termRef.current) {
      ...
      term.onKey(({ key, domEvent }) => {
        const printable = !domEvent.altKey && !domEvent.ctrlKey && !domEvent.metaKey && key.length === 1
