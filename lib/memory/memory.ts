...
export async function createMemoryThread(
  ...
  await db.execute({
    ...
    `,
    args: [thread_id, agent_id || null, network_id || null, name, JSON.stringify(metadata || {})],
  })
...
export async function generateThreadSummary(thread_id: string): Promise<string> {
  ...
  const conversationText = messages
    .map((msg) => {
      const role = msg.role === "tool" ? "Tool" : msg.role.charAt(0).toUpperCase() + msg.role.slice(1)
  ...
  const result = await generateAIResponse(provider, model_id, [systemPrompt, userPrompt], {
    apiKey: api_key,
    temperature: 0.3,
  })
