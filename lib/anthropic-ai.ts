...
import { streamText as streamTextFn, generateText as generateTextFn, convertToCoreMessages, generateId } from "ai"
...
export async function getModelConfig(modelId: string): Promise<ModelSettings | undefined> {
  ...
  const parsedModel: ModelSettings = {
    ...
    category: typeof model.category === 'string' ? (model.category as ModelCategory) : undefined,
    capabilities: isPlainObject(model.capabilities) ? (model.capabilities as unknown as ModelCapabilities) : undefined,
...
function normalizeMessages(messages: ChatMessage[]): Array<Omit<ChatMessage, "createdAt" | "role"> & { createdAt?: Date; role: SDKMessageRole; id: string }> {
  return messages
