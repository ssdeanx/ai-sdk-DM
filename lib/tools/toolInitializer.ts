...
export async function initializeTools(options: ToolInitializerOptions = {}) {
  ...
  try {
    ...
    const customTools = includeCustom
      ? await initializeCustomTools()
      : undefined;

    // Initialize agentic tools
    const agenticToolsResult = includeAgentic
    ...
    if (trace) {
      await logEvent({
        ...
        metadata: {
          level: 'info',
          builtInCount: builtInTools
            ? Object.keys(builtInTools).reduce(
                (acc, category) =>
                  acc + Object.keys(builtInTools[category] || {}).length,
                0
              )
            : 0,
          customCount: customTools ? Object.keys(customTools).length : 0,
          agenticCount: agenticToolsResult
            ? Object.keys(agenticToolsResult).length
            : 0,
          totalCategories: Object.keys(allInitializedTools).filter(
            (key) =>
              allInitializedTools[key as keyof typeof allInitializedTools] !==
              undefined
          ).length,
        },
      });
 ...
 */
export function initializeBuiltInTools(): Record<
  string,
  Record<string, Tool<any, any> | undefined>
> {
  const builtInToolGroups: Record<
    string,
    Record<string, Tool<any, any> | undefined>
  > = {};

  if (webTools && webTools.tools) {
    builtInToolGroups.web = webTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
  if (codeTools && codeTools.tools) {
    builtInToolGroups.code = codeTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
  if (dataTools && dataTools.tools) {
    builtInToolGroups.data = dataTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
  if (fileTools && fileTools.tools) {
    builtInToolGroups.file = fileTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
  if (apiTools && apiTools.tools) {
    builtInToolGroups.api = apiTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
  if (ragTools && ragTools.tools) {
    builtInToolGroups.rag = ragTools.tools as Record<
      string,
      Tool<any, any> | undefined
    >;
  }
 ...
 */
export function initializeAgenticTools(): Record<
  string,
  Tool<any, any> | undefined
> {
 ...
 */
export async function initializeCustomTools(): Promise<
  Record<string, { instance: any; category: string }>
> {
  ...
  try {
    ...
    const toolsFromDb = await getData<any>('tools', {
      // Assuming 'tools' table has a 'category' column
    ...
    for (const dbTool of toolsFromDb) {
      try {
        ...
        if (!parametersSchemaString) {
          console.warn(
            `Parameters schema not found for custom tool ${name}. Skipping.`
          );
          continue;
        }
        ...
        if (!implementation) {
          console.warn(
            `Implementation not found for custom tool ${name}. Skipping.`
          );
          continue;
        }
        ...
        customTools[name] = {
          instance: tool({
            ...
            execute: async (
              params: z.infer<typeof zodSchema>
            ): Promise<any> => {
              // Return type changed to Promise<any> for simplicity, can be more specific
              try {
                ...
                return {
                  tool_execution_error: `Tool execution failed: ${errorMessage}`,
                };
              }
            },
          }),
        ...
        // Explicitly type error
        console.error(
          `Error loading custom tool ${(dbTool as any)?.name || 'unknown'}:`,
          error
        );
      }
    }
    ...
    // Explicitly type error for the outer catch
