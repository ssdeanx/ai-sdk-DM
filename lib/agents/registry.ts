    ...
    try {
      ...
      const hooks: AgentHooks = {
        onStart: async (input, threadId) => {
          console.log(`Agent ${agentConfig.name} started with input: ${input.substring(0, 50)}...`)
        },
        onToolCall: async (toolName, params) => {
          console.log(`Agent ${agentConfig.name} called tool: ${toolName}`)
        },
        onFinish: async (result) => {
          console.log(`Agent ${agentConfig.name} finished with output: ${result.output.substring(0, 50)}...`)
        }
      }
