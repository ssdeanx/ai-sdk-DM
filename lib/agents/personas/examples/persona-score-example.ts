 ...
 */
async function getTopPerformingPersonasWithScoring(
  options: TopPerformingPersonasOptions = {}
): Promise<PersonaWithScore[]> {
  // Set default options
  const {
    limit = 5,
    minUsageCount = 3,
  ...
  try {
      ...
      filteredPersonas = allPersonas.filter((persona) => {
        ...
        return requiredCapabilities.every(
          (capability) =>
            ...
            (
              persona as PersonaDefinition & { specializations?: string[] }
            ).specializations?.includes(capability)
        );
      });
    }
      ...
      filteredPersonas.map(async (persona): Promise<PersonaWithScore> => {
        ...
        return {
          ...
          score: {
            ...
            token_usage_avg: score.token_usage_avg ?? {
              prompt_tokens: 0,
              completion_tokens: 0,
              total_tokens: 0,
            },
    ...
    let result = personasWithScores.filter((item) => {
      ...
      if (taskType) {
        ...
        const isSpecializedForTask =
          item.persona.tags?.includes(`task:${taskType}`) ||
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'latency':
          ...
          return (
            (a.score.average_latency_ms || Infinity) -
            (b.score.average_latency_ms || Infinity)
          );
    ...
    // Log the results
    console.log(
      `Retrieved ${result.length} top performing personas sorted by ${sortBy}`
    );
    result.forEach((item, index) => {
      console.log(
        `${index + 1}. ${item.persona.name} (${sortBy}: ${
          sortBy === 'latency'
 ...
 */
async function useTopPerformingPersonaForTask(
  taskType: string,
  requiredCapabilities: GeminiCapability[] = []
 ...
 */
async function analyzePersonaPerformanceTrend(
  personaId: string,
  ...
  console.log(
    `Analyzing performance trend for persona ${personaId} over ${timeFrameDays} days`
  );
  ...
  const olderAvg =
    olderHalf.reduce((sum, item) => sum + item.rating, 0) / olderHalf.length;
  const newerAvg =
    newerHalf.reduce((sum, item) => sum + item.rating, 0) / newerHalf.length;
 ...
 */
async function getBestPersonaForTaskType(
  taskType: string
): Promise<PersonaWithScore | null> {
  ...
  const supportingPersonas = allPersonas.filter(
    (persona) =>
      persona.tags?.includes(`task:${taskType}`) ||
      persona.tags?.includes(taskType)
  );
  ...
  personasWithScores.sort(
    (a, b) => b.score.overall_score - a.score.overall_score
  );

  // Filter personas that have been used for this task type
  const usedPersonas = personasWithScores.filter(
    (item) =>
      (item.score.metadata as any)?.task_specific_scores?.[taskType]
        ?.usage_count > 0
  );
  ...
  if (usedPersonas.length === 0) {
    ...
    const mostSpecialized = personasWithScores.sort((a, b) => {
      const aSpecialization = a.persona.metadata?.specializations?.includes(
        taskType
      )
        ? 1
        : 0;
      const bSpecialization = b.persona.metadata?.specializations?.includes(
        taskType
      )
        ? 1
        : 0;
    ...
    console.log(
      `Recommending ${mostSpecialized.persona.name} based on specialization`
    );
  ...
  usedPersonas.sort((a, b) => {
    const aScore =
      (a.score.metadata as any)?.task_specific_scores?.[taskType]
        ?.success_rate || 0;
    const bScore =
      (b.score.metadata as any)?.task_specific_scores?.[taskType]
        ?.success_rate || 0;
  ...
  console.log(
    `Task-specific success rate: ${(bestPersona.score.metadata as any)?.task_specific_scores?.[taskType]?.success_rate.toFixed(2) || 0}`
  );
  return bestPersona;
}
/**
 ...
 */
async function selectPersonaForTask(
  taskType: string,
  requiredCapabilities: GeminiCapability[] = []
) {
...
async function getMostUsedPersonas() {
  ...
  if (Array.isArray(mostUsedPersonas)) {
    mostUsedPersonas.forEach((item, index) => {
      console.log(
        `${index + 1}. ${item.persona.name} (Usage: ${item.score.usage_count})`
      );
      console.log(
        `   Score: ${item.score.overall_score}, Success Rate: ${item.score.success_rate}`
      );
    });
  }
...
async function runExample() {
  try {
    ...
    const success = await usePersonaForTask(
