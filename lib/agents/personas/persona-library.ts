...
export const GeminiModelIdSchema = z
  .string()
  .describe("A valid Gemini Model ID, e.g., 'models/gemini-1.5-pro'");
...
export const PersonaDefinitionSchema = z.object({
  ...
  modelSettings: z
    .record(z.any())
    .optional()
    .describe('e.g., { temperature: 0.7, topP: 0.9 }'),
  traits: z
    .array(z.string())
    .optional()
    .describe("e.g., ['concise', 'empathetic', 'technical']"),
  ...
  tags: z
    .array(z.string())
    .optional()
    .describe("e.g., ['customer-support', 'developer-tool']"),
  exampleDialogues: z.array(ExampleDialogueSchema).optional(),
  knowledgeBaseIds: z
    .array(z.string())
    .optional()
    .describe('IDs of knowledge bases to use'),
  preferredModels: z
    .array(GeminiModelIdSchema)
    .optional()
    .describe('Ordered list of preferred model IDs'),
  metadata: z.record(z.any()).optional().describe('Any other custom metadata'),
  compatibleMicroPersonas: z
    .array(z.string())
    .optional()
    .describe('IDs of compatible micro-personas'),
  lastUpdatedAt: z
    .string()
    .datetime()
    .optional()
...
export const MicroPersonaDefinitionSchema = z.object({
  id: z
    .string()
    .min(1, 'ID is required')
    .default(() => `micro-${uuidv4()}`),
  name: z.string().min(1, 'Name is required'),
  description: z.string().optional(),
  parentPersonaId: z
    .string()
    .min(1, 'Parent Persona ID is required')
    .optional(),
  promptFragment: z
    .string()
    .min(
      1,
      'Prompt fragment is required to augment or replace parts of the base prompt'
    ),
  modelSettingsOverrides: z
    .record(z.any())
    .optional()
    .describe('e.g., { temperature: 0.5 }'),
  tags: z
    .array(z.string())
    .optional()
    .describe("e.g., ['humor', 'conciseness']"),
  microTraits: z
    .array(z.string())
    .optional()
    .describe(
      "Specific traits for this micro-persona, e.g., ['sarcastic', 'formal']"
    ),
  conflictingMicroPersonaIds: z
    .array(z.string())
    .optional()
    .describe('IDs of micro-personas that should not be used with this one'),
  requiredCapabilities: z
    .array(z.nativeEnum(GeminiCapability))
    .optional()
    .describe(
      'Capabilities specifically required or enhanced by this micro-persona'
    ),
  metadata: z
    .record(z.any())
    .optional()
    .describe('Any other custom metadata for the micro-persona'),
  ...
  overrides: z
    .object({
      systemPromptTemplate: z
        .string()
        .optional()
        .describe('Completely replaces the base system prompt if provided'),
      modelSettings: z
        .record(z.any())
        .optional()
        .describe('Merges with and overrides base model settings'),
      traits: z
        .array(z.string())
        .optional()
        .describe('Adds to or replaces base traits'),
      capabilities: z
        .array(z.nativeEnum(GeminiCapability))
        .optional()
        .describe('Adds to or replaces base capabilities'),
      safetySettings: z
        .array(SafetySettingSchema)
        .optional()
        .describe('Replaces base safety settings'),
      tags: z
        .array(z.string())
        .optional()
        .describe('Adds to or replaces base tags'),
      exampleDialogues: z
        .array(ExampleDialogueSchema)
        .optional()
        .describe('Replaces base example dialogues'),
      knowledgeBaseIds: z
        .array(z.string())
        .optional()
        .describe('Adds to or replaces base knowledge base IDs'),
      preferredModels: z
        .array(GeminiModelIdSchema)
        .optional()
        .describe('Replaces base preferred models'),
      metadata: z
        .record(z.any())
        .optional()
        .describe('Merges with and overrides base metadata'),
    })
    .optional(),
  lastUpdatedAt: z
    .string()
...
export type MicroPersonaDefinition = z.infer<
  typeof MicroPersonaDefinitionSchema
>;
...
export function validatePersonaDefinition(data: unknown): PersonaDefinition {
  ...
  if (!result.success) {
    console.error(
      'PersonaDefinition validation failed:',
 ...
 * @throws ZodError if validation fails.
 */
export function validateMicroPersonaDefinition(
  data: unknown
): MicroPersonaDefinition {
  const result = MicroPersonaDefinitionSchema.safeParse(data);
  if (!result.success) {
    console.error(
      'MicroPersonaDefinition validation failed:',
      result.error.flatten().fieldErrors
    );
    throw result.error;
  }
...
export function composePersona(
  ...
  const basePersona = validatePersonaDefinition(
    JSON.parse(JSON.stringify(basePersonaInput))
  );

  if (!microPersonaInput) {
    return basePersona;
  }
  const microPersona = validateMicroPersonaDefinition(
    JSON.parse(JSON.stringify(microPersonaInput))
  );
  ...
  const microDesc = microPersona.description
  ...
  if (microPersona.overrides) {
    if (microPersona.overrides.systemPromptTemplate) {
      composed.systemPromptTemplate =
        microPersona.overrides.systemPromptTemplate;
    ...
    composed.modelSettings = {
      ...composed.modelSettings,
      ...microPersona.overrides.modelSettings,
    };
    composed.traits = microPersona.overrides.traits
      ? Array.from(
          new Set([
            ...(composed.traits || []),
            ...microPersona.overrides.traits,
          ])
        )
      : composed.traits;
    composed.capabilities = microPersona.overrides.capabilities
      ? Array.from(
          new Set([
            ...(composed.capabilities || []),
            ...microPersona.overrides.capabilities,
          ])
        )
      : composed.capabilities;
    composed.safetySettings =
      microPersona.overrides.safetySettings || composed.safetySettings;
    composed.tags = microPersona.overrides.tags
      ? Array.from(
          new Set([...(composed.tags || []), ...microPersona.overrides.tags])
        )
      : composed.tags;
    composed.exampleDialogues =
      microPersona.overrides.exampleDialogues || composed.exampleDialogues;
    composed.knowledgeBaseIds = microPersona.overrides.knowledgeBaseIds
      ? Array.from(
          new Set([
            ...(composed.knowledgeBaseIds || []),
            ...microPersona.overrides.knowledgeBaseIds,
          ])
        )
      : composed.knowledgeBaseIds;
    composed.preferredModels =
      microPersona.overrides.preferredModels || composed.preferredModels;
    composed.metadata = {
      ...composed.metadata,
      ...microPersona.overrides.metadata,
    };
  } else if (microPersona.promptFragment) {
  ...
  if (
    microPersona.modelSettingsOverrides &&
    (!microPersona.overrides || !microPersona.overrides.modelSettings)
  ) {
    composed.modelSettings = {
      ...composed.modelSettings,
      ...microPersona.modelSettingsOverrides,
  ...
  if (microPersona.microTraits) {
    composed.traits = Array.from(
      new Set([...(composed.traits || []), ...microPersona.microTraits])
    );
  }
  if (
    microPersona.tags &&
    (!microPersona.overrides || !microPersona.overrides.tags)
  ) {
    composed.tags = Array.from(
      new Set([...(composed.tags || []), ...microPersona.tags])
    );
  }
  if (
    microPersona.requiredCapabilities &&
    (!microPersona.overrides || !microPersona.overrides.capabilities)
  ) {
    composed.capabilities = Array.from(
      new Set([
        ...(composed.capabilities || []),
        ...microPersona.requiredCapabilities,
      ])
...
export const PersonaScoreSchema = z.object({
  id: z
    .string()
    .min(1)
    .default(() => uuidv4()), // Score record ID
  ...
  average_latency_ms: z
    .number()
    .min(0)
    .default(0)
    .describe('Average response latency in milliseconds'),
  user_satisfaction_avg: z
    .number()
    .min(0)
    .max(5)
    .default(0)
    .describe('Average user satisfaction score (e.g., 1-5 scale)'),
  ...
  adaptability_score: z
    ...
    .default(0)
    .describe(
      'Score indicating how well persona adapts to different contexts/tasks'
    ),
  cost_per_interaction_avg: z
    .number()
    .min(0)
    .default(0)
    .optional()
    .describe('Average cost if token/cost tracking is implemented'),
  token_usage_avg: z
    .object({
  ...
  overall_score: z
    .number()
    .default(0)
    .describe('Calculated overall performance score'),
  last_used_at: z.string().datetime().optional(),
  last_scored_at: z.string().datetime().optional(),
  metadata: z
    .record(z.any())
    .optional()
    .describe('Additional metadata for scoring, e.g., version tested'),
  created_at: z
    .string()
    .datetime()
    .default(() => new Date().toISOString()),
...
export const ScoreUpdateDataSchema = z.object({
  ...
  adaptabilityFactor: z
    .number()
    .min(-1)
    .max(1)
    .optional()
    .describe(
      'A factor indicating positive (1) or negative (-1) adaptation, or neutral (0)'
    ),
...
export const basePersonas: Record<string, PersonaDefinition> = {
  neutralAssistant: validatePersonaDefinition({
    ...
    description:
      'A general-purpose AI assistant that provides neutral and objective responses.',
    systemPromptTemplate:
      'You are a helpful and neutral AI assistant. Provide clear and concise answers. Respond factually and avoid expressing personal opinions unless explicitly asked for a subjective viewpoint clearly marked as such.',
  ...
  creativeWriter: validatePersonaDefinition({
    ...
    description:
      'An AI assistant specialized in generating creative content, stories, and scripts.',
    systemPromptTemplate:
      "You are a highly creative AI writer. Generate imaginative and engaging content based on the user's request. Emphasize storytelling, vivid descriptions, and emotional depth. Feel free to explore unconventional ideas.",
    ...
    capabilities: [
      GeminiCapability.CREATIVE_WRITING,
      GeminiCapability.STORYTELLING,
      GeminiCapability.TEXT_GENERATION,
    ],
    exampleDialogues: [
      {
        user: 'Write a short story about a dragon who loves to bake.',
        assistant:
          'In a cozy cavern, warmed by his own gentle flames, lived Ignis, a dragon with a peculiar passion: baking. His massive claws, surprisingly deft, kneaded dough with the rhythm of an ancient song, and his fiery breath was carefully controlled to produce the perfect golden crust on his famous sunberry scones...',
      },
    ],
  }),
  technicalExpert: validatePersonaDefinition({
    ...
    description:
      'An AI assistant providing in-depth technical explanations and analysis.',
    systemPromptTemplate:
      'You are a seasoned technical expert with deep knowledge in your domain. Provide precise, accurate, and detailed explanations. Cite sources or methodologies where appropriate. Break down complex topics into understandable components.',
    ...
    capabilities: [
      ...
      GeminiCapability.ADVANCED_REASONING,
    ],
  }),
};
...
export const specializedPersonas: Record<string, PersonaDefinition> = {
  codeHelper: validatePersonaDefinition({
    ...
    name: 'Code Helper',
    description:
      'An AI assistant that helps with programming questions, debugging, and code generation across multiple languages.',
    systemPromptTemplate:
    ...
    safetySettings: [
      {
        category: GeminiSafetyCategory.HARM_CATEGORY_DANGEROUS_CONTENT,
        threshold: GeminiSafetyThreshold.BLOCK_MEDIUM_AND_ABOVE,
      },
    ],
    exampleDialogues: [
      {
        user: 'How do I sort a list of dictionaries in Python by a specific key?',
        assistant:
          "You can sort a list of dictionaries in Python using the `sorted()` function with a `lambda` function as the `key`. For example, to sort by the 'age' key: `sorted_list = sorted(my_list, key=lambda x: x['age'])`.",
      },
    ],
  }),
  customerSupportAgent: validatePersonaDefinition({
    ...
    description:
      'A friendly and empathetic AI agent for handling customer inquiries and support with a focus on resolution.',
    systemPromptTemplate:
      'You are a friendly, patient, and highly helpful customer support agent. Your primary goal is to resolve customer issues effectively and efficiently. Listen carefully to customer concerns, provide clear, step-by-step solutions, and maintain a positive and empathetic tone throughout the interaction. If you cannot resolve an issue, explain why and clearly state the next steps or how to escalate the issue.',
    ...
    ], // FUNCTION_CALLING for tools like order lookup
    safetySettings: [
      {
        category: GeminiSafetyCategory.HARM_CATEGORY_HARASSMENT,
        threshold: GeminiSafetyThreshold.BLOCK_MEDIUM_AND_ABOVE,
      },
      {
        category: GeminiSafetyCategory.HARM_CATEGORY_HATE_SPEECH,
        threshold: GeminiSafetyThreshold.BLOCK_MEDIUM_AND_ABOVE,
      },
    ],
    exampleDialogues: [
      {
        user: "My order hasn't arrived yet, it's three days late!",
        assistant:
          "I'm so sorry to hear your order is delayed! I understand how frustrating that can be. Could you please provide me with your order number so I can check its status for you right away?",
      },
    ],
  }),
  researchAnalyst: validatePersonaDefinition({
    ...
    description:
      'An AI assistant for conducting research, summarizing information, and identifying key insights from various data sources.',
    systemPromptTemplate:
      'You are a meticulous and insightful research analyst. Your task is to gather, synthesize, and analyze information from provided sources or by using available tools. Identify key findings, trends, and potential biases. Present your analysis in a clear, structured, and objective manner. Always cite your sources if information is retrieved externally.',
