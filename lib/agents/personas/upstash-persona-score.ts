 ...
 */
export async function savePersonaScore(
  personaId: string,
  score: PersonaScore
): Promise<void> {
  ...
  try {
    ...
    pipeline.zadd(PERSONA_SCORE_INDEX, {
    ...
    throw new PersonaScoreError(
      `Failed to save persona score for ${personaId}`,
      error
    );
  }
}
 ...
 */
export async function loadPersonaScore(
  personaId: string
  ...
  try {
    ...
    throw new PersonaScoreError(
      `Failed to load persona score for ${personaId}`,
      error
    );
  }
}
 ...
 */
export async function updatePersonaScore(
  personaId: string,
  ...
  try {
    ...
    if (updateData.success !== undefined) {
      ...
      currentScore.success_rate =
        ((currentScore.success_rate || 0) * (currentScore.usage_count - 1) +
          successValue) /
        currentScore.usage_count;
    }

    // Update user satisfaction if provided
    if (updateData.userSatisfaction !== undefined) {
      currentScore.user_satisfaction =
        ((currentScore.user_satisfaction || 0) *
          (currentScore.usage_count - 1) +
          updateData.userSatisfaction) /
        currentScore.usage_count;
    }

    // Update adaptability if provided
    if (updateData.adaptabilityFactor !== undefined) {
      currentScore.adaptability =
        ((currentScore.adaptability || 0) * (currentScore.usage_count - 1) +
          updateData.adaptabilityFactor) /
        currentScore.usage_count;
    }

    // Update latency if provided
    if (updateData.latency !== undefined) {
      currentScore.latency =
        ((currentScore.latency || 0) * (currentScore.usage_count - 1) +
          updateData.latency) /
        currentScore.usage_count;
    }
    ...
    if (updateData.taskType && updateData.taskScore !== undefined) {
      ...
      const currentTaskScore = taskScores[updateData.taskType] || {
        score: 0,
        count: 0,
      };

      currentTaskScore.score =
        (currentTaskScore.score * currentTaskScore.count +
          updateData.taskScore) /
    ...
    if (updateData.previousOverallScore !== undefined) {
      const scoreDiff =
        currentScore.overall_score - updateData.previousOverallScore;
      if (scoreDiff > 0.05) {
    ...
    throw new PersonaScoreError(
      `Failed to update persona score for ${personaId}`,
      error
    );
  }
}
...
export async function recordPersonaUsage(
  ...
  try {
    ...
    throw new PersonaScoreError(
      `Failed to record persona usage for ${personaId}`,
      error
    );
  }
}
