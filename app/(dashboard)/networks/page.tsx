...
import { AlertCircle, Loader2, Network as NetworkIcon, Plus, Play, Settings, Trash } from "lucide-react"
...
export default function NetworksPage() {
  ...
  async function onSubmit(values: z.infer<typeof networkFormSchema>) {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred while saving the network",
        variant: "destructive",
      })
    } finally {
  ...
  async function onAgentSubmit(values: z.infer<typeof agentAssignmentSchema>) {
    ...
    try {
      ...
      const response = await fetch(`/api/networks/${selectedNetwork.id}/agents`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred while adding the agent",
        variant: "destructive",
      })
    } finally {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred while deleting the network",
        variant: "destructive",
      })
    } finally {
      setIsDeleting(false)
  ...
  async function fetchNetworkAgents(networkId: string) {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred while fetching network agents",
        variant: "destructive",
      })
      setNetworkAgents([])
    } finally {
  ...
  async function handleRemoveAgent(agentId: string) {
    ...
    try {
      const response = await fetch(`/api/networks/${selectedNetwork.id}/agents/${agentId}`, {
        method: "DELETE",
      })
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "An error occurred while removing the agent",
        variant: "destructive",
      })
    } finally {
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card className="col-span-full p-6">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-muted-foreground mt-1">
                You haven't created any networks yet. Click the "Create New Network" button to get started.
              </p>
            </div>
          </Card>
        </div>
                    ...
                    <tbody>
                      {networkAgents.map((agent) => (
                        <tr key={agent.id} className="border-b last:border-0">
                          <td className="px-4 py-2">{agent.agents?.name || "Unknown Agent"}</td>
