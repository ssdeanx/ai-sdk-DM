...
async function executeCodeTool(name: string, parameters: any) {
  switch (name.toLowerCase()) {
    case 'codeexecute':
    case 'code_execute':
      return await executeCodeExecution(
        parameters.code,
        parameters.language,
    ...
    case 'codeanalyze':
    case 'code_analyze':
      return await executeCodeAnalysis(
        parameters.code,
        parameters.language,
        parameters.analysis || ['complexity']
      );
    default:
...
async function executeDataTool(name: string, parameters: any) {
  switch (name.toLowerCase()) {
    ...
    case 'datavisualize':
    case 'data_visualize':
      return await executeDataVisualization(
        parameters.data,
        parameters.type,
        parameters.options
      );
...
async function executeWebExtract(url: string, selector?: string) {
  ...
  console.log(
    `Extracting content from: ${url}, selector: ${selector || 'none'}`
  );
...
async function executeCodeAnalysis(
  ...
  if (analysis.includes('complexity')) {
    ...
    results['complexity'] = {
      ...
      nestingLevel,
      assessment:
        nestingLevel > 5 ? 'High complexity' : 'Acceptable complexity',
    };
  }
...
async function executeDataVisualization(
  data: any,
  type: string,
  options: any = {}
) {
