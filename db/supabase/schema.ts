...
export const models = pgTable('models', {
  ...
  provider_id: uuid('provider_id')
    .notNull()
    .references(() => providers.id, { onDelete: 'restrict' }), // FK to providers
  ...
  default_temperature: numeric('default_temperature', {
    precision: 3,
    scale: 2,
  })
    .notNull()
    .default('0.7'),
  default_top_p: numeric('default_top_p', { precision: 3, scale: 2 })
    .notNull()
    .default('1.0'),
...
export const agents = pgTable('agents', {
  ...
  model_id: uuid('model_id')
    .notNull()
    .references(() => models.id, { onDelete: 'restrict' }), // Restrict deletion if model is in use
  system_prompt: text('system_prompt'),
  persona_id: uuid('persona_id').references(() => agent_personas.id, {
    onDelete: 'set null',
  }),
  ...standardTimestamps(),
});
...
export const content_table = pgTable('content', {
  // Renamed to avoid conflict with 'content' column name
  id: uuid('id').primaryKey().defaultRandom(),
  type: text('type').notNull(),
  title: text('title'),
...
export const system_metrics = pgTable('system_metrics', {
  ...
  average_response_time_ms: numeric('average_response_time_ms', {
    precision: 10,
    scale: 2,
  }),
...
export const model_evaluations = pgTable('model_evaluations', {
  ...
  evaluation_date: timestamp('evaluation_date', { withTimezone: true })
    .notNull()
    .defaultNow(),
...
export const evaluation_metrics = pgTable('evaluation_metrics', {
  ...
  evaluation_id: uuid('evaluation_id')
    .notNull()
    .references(() => model_evaluations.id, { onDelete: 'cascade' }),
...
export const persona_scores = pgTable('persona_scores', {
  ...
  persona_id: uuid('persona_id')
    .notNull()
    .references(() => agent_personas.id, { onDelete: 'cascade' }),
  usage_count: integer('usage_count').notNull().default(0),
  success_rate: numeric('success_rate', { precision: 5, scale: 4 })
    .notNull()
    .default('0'),
  average_latency_ms: numeric('average_latency_ms', { precision: 10, scale: 2 })
    .notNull()
    .default('0'),
  user_satisfaction: numeric('user_satisfaction', { precision: 5, scale: 2 })
    .notNull()
    .default('0'), // e.g., scale of 1-5
  adaptability_score: numeric('adaptability_score', { precision: 5, scale: 2 })
    .notNull()
    .default('0'),
...
export const messages = pgTable('messages', {
  ...
  thread_id: uuid('thread_id')
    .notNull()
    .references(() => memory_threads.id, { onDelete: 'cascade' }),
...
export const database_transactions = pgTable('database_transactions', {
  id: uuid('id').primaryKey().defaultRandom(),
  connection_id: uuid('connection_id').references(
...
export const database_queries = pgTable('database_queries', {
  id: uuid('id').primaryKey().defaultRandom(),
  transaction_id: uuid('transaction_id').references(
    () => database_transactions.id,
    { onDelete: 'cascade' }
  ),
...
export const modelsRelations = relations(models, ({ many, one }) => ({
  ...
  modelPerformances: many(model_performance, {
    relationName: 'ModelPerformances',
  }),
  modelCosts: many(model_costs, { relationName: 'ModelCosts' }),
  modelEvaluations: many(model_evaluations, {
    relationName: 'ModelEvaluations',
  }),
  provider: one(providers, {
    fields: [models.provider_id],
    references: [providers.id],
    relationName: 'ProviderModels',
  }),
}));
...
export const agentsRelations = relations(agents, ({ one, many }) => ({
  model: one(models, {
    fields: [agents.model_id],
    references: [models.id],
    relationName: 'ModelAgents',
  }),
  persona: one(agent_personas, {
    fields: [agents.persona_id],
    references: [agent_personas.id],
    relationName: 'PersonaAgents',
  }),
...
export const workflowStepsRelations = relations(workflow_steps, ({ one }) => ({
  workflow: one(workflows, {
    fields: [workflow_steps.workflow_id],
    references: [workflows.id],
    relationName: 'WorkflowSteps',
  }),
  agent: one(agents, {
    fields: [workflow_steps.agent_id],
    references: [agents.id],
    relationName: 'AgentWorkflowSteps',
  }),
}));

export const agentToolsRelations = relations(agent_tools, ({ one }) => ({
  agent: one(agents, {
    fields: [agent_tools.agent_id],
    references: [agents.id],
    relationName: 'AgentToolsJunction',
  }),
  tool: one(tools, {
    fields: [agent_tools.tool_id],
    references: [tools.id],
    relationName: 'ToolAgentsJunction',
  }),
}));
...
export const blogPostsRelations = relations(blog_posts, ({ one }) => ({
  author: one(users, {
    ...
    references: [users.id],
    relationName: 'UserBlogPosts',
  }),
}));

export const mdxDocumentsRelations = relations(mdx_documents, ({ one }) => ({
  user: one(users, {
    fields: [mdx_documents.user_id],
    references: [users.id],
    relationName: 'UserMdxDocuments',
  }),
}));
...
export const tracesRelations = relations(traces, ({ one, many }) => ({
  user: one(users, {
    fields: [traces.user_id],
    references: [users.id],
    relationName: 'UserTraces',
  }),
...
export const spansRelations = relations(spans, ({ one, many }) => ({
  trace: one(traces, {
    fields: [spans.trace_id],
    references: [traces.id],
    relationName: 'TraceSpans',
  }),
  parentSpan: one(spans, {
    fields: [spans.parent_span_id],
    references: [spans.id],
    relationName: 'SpanHierarchy',
  }),
...
export const eventsRelations = relations(events, ({ one }) => ({
  trace: one(traces, {
    ...
    references: [traces.id],
    relationName: 'TraceEventsDirect',
  }),
  span: one(spans, {
    fields: [events.span_id],
    references: [spans.id],
    relationName: 'SpanEvents',
  }),
}));
...
export const modelPerformanceRelations = relations(
  ...
  ({ one }) => ({
    model: one(models, {
      fields: [model_performance.model_id],
      references: [models.id],
      relationName: 'ModelPerformances',
    }),
  })
...
export const modelCostsRelations = relations(model_costs, ({ one }) => ({
  model: one(models, {
    fields: [model_costs.model_id],
    references: [models.id],
    relationName: 'ModelCosts',
  }),
}));
...
export const modelEvaluationsRelations = relations(
  ...
  ({ one, many }) => ({
    model: one(models, {
      fields: [model_evaluations.model_id],
      references: [models.id],
      relationName: 'ModelEvaluations',
    }),
...
export const evaluationMetricsRelations = relations(
  evaluation_metrics,
  ({ one }) => ({
    evaluation: one(model_evaluations, {
      fields: [evaluation_metrics.evaluation_id],
      references: [model_evaluations.id],
      relationName: 'EvaluationMetrics',
    }),
  })
);
...
export const personaScoresRelations = relations(persona_scores, ({ one }) => ({
  persona: one(agent_personas, {
    fields: [persona_scores.persona_id],
    references: [agent_personas.id],
    relationName: 'PersonaScores',
  }),
}));
...
export const evaluationExamplesRelations = relations(
  ...
  ({ one }) => ({
    evaluation: one(model_evaluations, {
      fields: [evaluation_examples.evaluation_id],
      references: [model_evaluations.id],
      relationName: 'EvaluationExamples',
    }),
  })
);

export const memoryThreadsRelations = relations(
  memory_threads,
  ({ one, many }) => ({
    user: one(users, {
      fields: [memory_threads.user_id],
      references: [users.id],
      relationName: 'UserMemoryThreads',
    }),
    agent: one(agents, {
      fields: [memory_threads.agent_id],
      references: [agents.id],
      relationName: 'AgentMemoryThreads',
    }),
...
export const messagesRelations = relations(messages, ({ one }) => ({
  thread: one(memory_threads, {
    fields: [messages.thread_id],
    references: [memory_threads.id],
...
export const databaseConnectionsRelations = relations(
  ...
  ({ many }) => ({
    transactions: many(database_transactions, {
      relationName: 'ConnectionTransactions',
    }),
  })
);
...
export const databaseTransactionsRelations = relations(
  ...
  ({ one, many }) => ({
    connection: one(database_connections, {
      fields: [database_transactions.connection_id],
...
export const databaseQueriesRelations = relations(
  database_queries,
  ({ one }) => ({
    transaction: one(database_transactions, {
      fields: [database_queries.transaction_id],
      references: [database_transactions.id],
      relationName: 'TransactionQueries',
    }),
  })
);
...
export const scheduledTaskRunsRelations = relations(
  scheduled_task_runs,
  ({ one }) => ({
    task: one(scheduled_tasks, {
      fields: [scheduled_task_runs.task_id],
      references: [scheduled_tasks.id],
      relationName: 'TaskRuns',
    }),
  })
);
